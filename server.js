...
app.get('/player_info/post',(req,res)=>{
  ...
  var specific = "select * from football_players where id = '" + player + "';";
  var games_played = "SELECT COUNT(*) FROM football_games WHERE '" + player + "' = any(players);";
  db.task('get-everything', task => {
        return task.batch([
    ...
    .catch(error => {
        // display error message in case an error
            request.flash('error', err);
            response.render('pages/player_info/select_player', {
                title: 'Team Stats Page'
            })
    });
})
